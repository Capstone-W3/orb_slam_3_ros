...
#include <orb_slam3_ros/ORBState.h>
...
ROSPublisher::ROSPublisher(Map *map, double frequency, ros::NodeHandle nh) :
    ...
    octomap_(PublisherUtils::getROSParam<float>(nh, "/orb_slam3_ros/octomap/resolution", 0.1)),
    ...
    publish_octomap_(false), publish_projected_map_(true), publish_gradient_map_(false)
{
    ...
    orb_state_.state = orb_slam3_ros::ORBState::UNKNOWN;
    ...
    state_pub_       = nh_.advertise<orb_slam3_ros::ORBState>("info/state", 10);
...
void ROSPublisher::initializeParameters(ros::NodeHandle &nh) {
  ...
  nh.param<float>("/orb_slam3_ros/topic/orb_state_republish_rate", orb_state_republish_rate_, 1);

  // odom topic defined in ScaleCorrector.cpp
  nh.param<bool>("/orb_slam3_ros/map_scale/perform_correction",        perform_scale_correction_,  true);
  nh.param<float>("/orb_slam3_ros/map_scale/scaling_distance",         scaling_distance_,          1.000);
  nh.param<float>("/orb_slam3_ros/map_scale/set_manually",             map_scale_,                 1.500);
  nh.param<float>("/orb_slam3_ros/map_scale/camera_height",            camera_height_,             0.205);
  nh.param<float>("/orb_slam3_ros/map_scale/camera_height_multiplier", camera_height_mult_,        1.000);

  nh.param<std::string>("/orb_slam3_ros/frame/map_frame",          map_frame_,          ROSPublisher::DEFAULT_MAP_FRAME);
  nh.param<std::string>("/orb_slam3_ros/frame/map_frame_adjusted", map_frame_adjusted_, "/orb_slam3/odom");
  nh.param<std::string>("/orb_slam3_ros/frame/camera_frame",       camera_frame_,       ROSPublisher::DEFAULT_CAMERA_FRAME);
  nh.param<std::string>("/orb_slam3_ros/frame/base_frame",         base_frame_,         "/orb_slam3/base_link");

  nh.param<bool>("/orb_slam3_ros/octomap/enabled",                octomap_enabled_,        true);
  nh.param<bool>("/orb_slam3_ros/octomap/publish_octomap",        publish_octomap_,        false);
  nh.param<bool>("/orb_slam3_ros/octomap/publish_projected_map",  publish_projected_map_,  true);
  nh.param<bool>("/orb_slam3_ros/octomap/publish_gradient_map",   publish_gradient_map_,   false);

  nh.param<bool>("/orb_slam3_ros/octomap/rebuild",  octomap_rebuild_, false);
  nh.param<float>("/orb_slam3_ros/octomap/rate",    octomap_rate_,    1.0);
  // resolution is set default in constructor

  nh.param<double>("/orb_slam3_ros/occupancy/projected_map/min_height", projection_min_height_,  -10.0);
  nh.param<double>("/orb_slam3_ros/occupancy/projected_map/max_height", projection_max_height_,  +10.0);

  nh.param<int>   ("/orb_slam3_ros/occupancy/projected_map/morpho_oprations/erode_se_size",  erode_se_size_,  3);
  nh.param<int>   ("/orb_slam3_ros/occupancy/projected_map/morpho_oprations/erode_nb",       erode_nb_,       1);
  nh.param<int>   ("/orb_slam3_ros/occupancy/projected_map/morpho_oprations/open_se_size",   open_se_size_,   3);
  nh.param<int>   ("/orb_slam3_ros/occupancy/projected_map/morpho_oprations/open_nb",        open_nb_,        1);
  nh.param<int>   ("/orb_slam3_ros/occupancy/projected_map/morpho_oprations/close_se_size",  close_se_size_,  3);
  nh.param<int>   ("/orb_slam3_ros/occupancy/projected_map/morpho_oprations/close_nb",       close_nb_,       1);
  nh.param<int>   ("/orb_slam3_ros/occupancy/projected_map/morpho_oprations/erode2_se_size", erode2_se_size_, 3);
  nh.param<int>   ("/orb_slam3_ros/occupancy/projected_map/morpho_oprations/erode2_nb",      erode2_nb_,      1);

  nh.param<float> ("/orb_slam3_ros/occupancy/height_gradient_map/max_height",   gradient_max_height_,   0);
  nh.param<int>   ("/orb_slam3_ros/occupancy/height_gradient_map/nb_erosions",  gradient_nb_erosions_,  1);
  nh.param<float> ("/orb_slam3_ros/occupancy/height_gradient_map/low_slope",    gradient_low_slope_,    M_PI / 4.0);
  nh.param<float> ("/orb_slam3_ros/occupancy/height_gradient_map/high_slope",   gradient_high_slope_,   M_PI / 3.0);
  ...
  // DEPRECATED
  // nh.param<bool>("/orb_slam3_ros/octomap/tf_based", octomap_tf_based_, false);
  // nh.param<bool>("/orb_slam3_ros/frame/align_map_to_cam_frame",   align_map_to_cam_frame_, true);
  // nh.param<bool>("/orb_slam3_ros/frame/adjust_map_frame",          adjust_map_frame_,      false);
  // nh.param<float>("/orb_slam3_ros/topic/loop_close_republish_rate_", loop_close_republish_rate_, ROSPublisher::LOOP_CLOSE_REPUBLISH_RATE);
...
void ROSPublisher::octomapWorker()
{
    ...
    while (orb_state_.state != orb_slam3_ros::ORBState::OK)
    {
...
void ROSPublisher::Run()
{
    ...
    if ( perform_scale_correction_ && GetSystem()->GetSensorType() == ORB_SLAM3::System::eSensor::MONOCULAR) {
      ...
      while ( GetSystem()->GetTrackingState() <= ORB_SLAM3::Tracking::NOT_INITIALIZED ) {
        ROS_INFO("Waiting");
        if ( isStopRequested() ) {
      ...
      while ( !scale_corrector.isScaleUpdated() ) {
        ...
        if ( GetSystem()->GetTrackingState() == ORB_SLAM3::Tracking::LOST ) {
